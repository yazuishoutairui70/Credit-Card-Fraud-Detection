...
###  1.3 异常数据分析 (Anomaly Investigation)
...
### 2.1 宏观分类特征分析 (Categorical Features Analysis)
...
### 2.2 交易金额特征分析 (Transaction Amount Analysis)
...
### 2.3 客户生命周期与地址一致性分析 (Account Lifecycle & Address Analysis)
...
### 2.4 特征相关性分析 (Feature Correlation Analysis)
...
## 🛠️ Section 3：特征工程与矩阵净化 (Feature Engineering & Matrix Purification)
...
* **目标：** 构建一套纯净、无冗余、业务可解释的特征矩阵，最大化模型对欺诈信号的捕捉能力。
...
#### 离散变量编码与维度控制
* **处理动作：** 对低基数分类特征（`Device Used`、`Payment Method`、`Product Category`）进行 One-Hot Encoding，并设置 `drop_first=True` 避免虚拟变量陷阱。
* **逻辑考量：** 确保模型输入全部为数值型，同时严格控制共线性风险。
